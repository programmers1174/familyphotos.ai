...
class PersonReference:
    ...
    # Optionally keep all samples for debugging or more lenient matching later.
...
def build_references(cfg: EnrollConfig) -> list[PersonReference]:
    ...
    for person, image_paths in cfg.people.items():
        ...
        for img_path in image_paths:
            ...
            if len(faces) != 1:
                # Per your constraint: enrollment images should have exactly one face.
                # If multiple, fall back to the largest face to keep the pipeline runnable.
    ...
    *,
    ...
    for img_path in image_paths:
        faces = represent_faces(
            ...
            model_name=cfg.model.model_name,
            detector_backend=cfg.model.detector_backend,
            enforce_detection=False,  # allow photos with no faces
        )
